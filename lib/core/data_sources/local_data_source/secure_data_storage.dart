...
class SecureDataStorage {
  ...
  Future<void> setAppLanguage(final Locale locale) async {
    await _storage.write(key: languageLocale, value: locale.languageCode);
  }

  /// Get theme mode from secure storage
  Future<ThemeMode> getThemeMode() async {
    final String? mode = await _storage.read(key: themeMode);
    return mode == ThemeMode.light.name ? ThemeMode.light : ThemeMode.light;
  }

  Future<Locale?> getLocaleLanguage() async {
    final String? code = await _storage.read(key: languageLocale);
