...
class FailedPhotoReviewScreen extends StatefulWidget {
  final List<FailedPhotoInfo> failedPhotos;

  const FailedPhotoReviewScreen({
    super.key,
    required this.failedPhotos,
  });

  @override
  State<FailedPhotoReviewScreen> createState() =>
      _FailedPhotoReviewScreenState();
}

class _FailedPhotoReviewScreenState extends State<FailedPhotoReviewScreen> {
  ...
  void _onSkip() {
    if (_currentPage < widget.failedPhotos.length - 1) {
      ...
      // All reviewed, go back
      _onComplete();
    }
  }

  void _onManualAdd(FailedPhotoInfo failedPhoto) async {
    final result = await context.router.push(
      ClosetItemFormScreenRoute(imageFile: failedPhoto.imageFile),
    );
  ...
  Widget _buildPhotoPage(FailedPhotoInfo failedPhoto) {
    return Padding(
